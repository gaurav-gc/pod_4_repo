...
class SubwayStation(Station):
    ...
    def show_info(self):
        print(f'{self.station_name} station is located at {self.location} and it has these subway lines: {self.lines}')
...
lines: ['1', '2', '3', 'L']
'''
subway_station1 = SubwayStation("Union Sq.", "New York City", [
                                'A', 'C', 'L', 'N', 'Q', 'R'])
...
class BusStation(Station):
    ...
    def show_info(self):
        if self.open:
            print(
                f'{self.station_name} station is located at {self.location} and it has these routes: {self.routes}')
        else:
...
'''
nyc_megabus_station = BusStation(station_name='NYC Megabus Stop',
