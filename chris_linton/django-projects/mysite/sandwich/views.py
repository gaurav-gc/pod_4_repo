...
def full_menu(request):
    if request.method == 'GET':
        """ Build a each type of sandwich """
        sandwiches = []
        count = 0
        for m in ingredients['meats']:
            meat = ingredients['meats'][count]
            count += 1
            count2 = 0
            for c in ingredients['cheeses']:
                cheese = ingredients['cheeses'][count2]
                count2 += 1
                count3 = 0
                for t in ingredients['toppings']:
                    topping = ingredients['toppings'][count3]
                    count3 += 1
                    sandwich = f'{meat} and {cheese} with {topping}'
                    sandwiches.append(sandwich)
        return render(request, 'full_menu.html', context={ 'sandwiches': sandwiches })