...
print(f"1.0 The variable 'books' is a list defined both by the funtion 'type' {type(books)} as well as the square brackets that surround the containing list.\n")
...
def search_by_author(name):
    ...
    for book in books_with_details:
        if name in book['author']:
            selection.append(book['title'])
    return selection
# 2.4
# Search for book titles by the author 'Timothy Snyder' using the search_by_author function
print("2.4\n")
snyder_books = search_by_author('timothy snyder')
print("Timothy Snyder books:")
for book in snyder_books:
    print(book)
print()